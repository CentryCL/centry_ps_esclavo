 ...
 */
  public static function productSave($product_ps,$product,$sync){
    ...
    if(count($product->variants) > 1){
      ...
      if(Configuration::get("CENTRY_SYNC_VARIANT_SIMPLE")){
    ...
    private function characteristics($product,$product_ps,$sync){
      ...
      if($sync["warranty"]){
          ...
          $id = FeatureCentry::getId("Garantía")[0]["id"];
          array_push($erase,$id);
        }
      }
      if($sync["characteristics"]){
        if(property_exists($product,"seasonyear")&&$product->seasonyear){
          ...
          $id = FeatureCentry::getId("Año Temporada")[0]["id"];
          array_push($erase,$id);
        }
        ...
        if(property_exists($product,"season")&&$product->season){
          ...
          $id = FeatureCentry::getId("Temporada")[0]["id"];
          array_push($erase,$id);
        }
        ...
        if(property_exists($product,"gender_name")&&$product->gender_name){
          ...
          $id = FeatureCentry::getId("Género")[0]["id"];
          array_push($erase,$id);
        }
      }
    ...
    private function saveImages($product_ps,$product){
      ...
      foreach($product->assets as $asset){
        if (!ImageCentry::getId($asset->_id)[0]["id"]){
    ...
    private function deleteUnconnectedPhotos($product_ps,$product){
      ...
      foreach($product->assets as $image){
        $img = ImageCentry::getId($image->_id)[0]["id"];
        if($img){
    ...
    private function deleteUnconnectedVariants($product_ps,$variants){
      ...
      foreach($variants as $variant){
        $id_comb = ProductCentry::getId($variant->_id)[0]["id"];
        if($id_comb){
